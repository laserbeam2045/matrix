...
# 戻り値：子集合の数(存在しなければ0)
DELIMITER //
  DROP FUNCTION IF EXISTS get_children_count//
CREATE FUNCTION get_children_count (pID INT) RETURNS INT
BEGIN
  DECLARE cnt INT;
...
BEGIN
  ...
  SET childrenCnt = get_children_count(pID);
...
# 戻り値：0以上の整数
DELIMITER //
  DROP FUNCTION IF EXISTS get_include_count//
CREATE FUNCTION get_include_count (outsideID INT, insideID INT) RETURNS INT
BEGIN
...
# 戻り値: 0(=成功)または1(=失敗)
DELIMITER //
  DROP FUNCTION IF EXISTS move_set//
CREATE FUNCTION move_set (cID INT, pID INT, idx INT) RETURNS INT
BEGIN
  # 集合(id=cID)の中に、集合(id=pID)が存在しない場合のみ更新する
  IF 0 = get_include_count(cID, pID) THEN
    # 挿入可能な位置を取得する
...
# 戻り値: 0(=成功)または1(=失敗)
DELIMITER //
  DROP FUNCTION IF EXISTS move_sets//
CREATE FUNCTION move_sets (cID INT, pID INT, idx INT) RETURNS INT
BEGIN
  ...
  # 最初に指定要素だけ移動させる
  SET result = move_set(cID, pID, idx);

  # ループ処理を開始
  read_loop: LOOP
    FETCH cur INTO pID, cID, _;
    IF done OR (0 <> result) THEN
      LEAVE read_loop;
    END IF;
    # 子孫要素を逐次移動させる
    SET result = move_set(cID, pID, -1);
  END LOOP;
